...
contract HeartHeads is ERC721, ERC721URIStorage, Ownable {
    ...
    function removeTokenURI(string memory uri) internal {
        for (uint256 i = 0; i < tokenURIs.length; i++) {
            if (keccak256(abi.encodePacked(tokenURIs[i])) == keccak256(abi.encodePacked(uri))) {
    ...
    function addRandomTokenURIs(string[] memory newTokenURIs, uint256[] memory limits) external onlyOwner {
        require(newTokenURIs.length == limits.length, "Length mismatch between tokenURIs and limits");
    ...
    function getRandomTokenURI() internal view returns (string memory) {
        ...
        uint256 randomIndex = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % tokenURIs.length;
        return tokenURIs[randomIndex];
    }
    function setNftPrice(uint256 newPrice)  onlyOwner public{
                nftprice = newPrice;
    ...
    function updateMetadataUrl(uint256 tokenId, string memory newMetadataUrl) public  {
